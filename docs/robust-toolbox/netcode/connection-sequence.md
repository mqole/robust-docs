...
sequenceDiagram
    ...
    rect
        ...
        Note over S: Connecting event is raised,content can deny connection if desired
        S->>C: MsgLoginSuccess
    ...
    Note over S,C: NetManager.Connected eventNon-handshake messages are now allowed
    C-->>S: MsgConVars
    C-->>S: MsgConCmdReg
    par PlayerManager.NewSession
        Note over S: Server-side player session gets created,PlayerStatusChanged gets ran for first time
        S->>C: MsgSyncTimeBase
    ...
    S->>C: MsgPlayerListContains Connected Status
